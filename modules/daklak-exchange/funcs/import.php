...
if (!empty($action)) {
  ...
  switch ($action) {
    ...
    case 'get-import':
      ...
      $sql = "select * from pet_daklak_import where id = $id";
      $query = $db->query($sql);
      $import = $query->fetch();
      $source = getSource($import['sourceid']);

      $sql = "select a.number, a.price, b.id, b.code, b.name from pet_daklak_import_row a inner join pet_daklak_product b on a.itemid = b.id where importid = $id";
      $query = $db->query($sql);
      $list = array();
    ...
    case 'update-import':
       ...
       * 3. cập nhật thay đổi trên import_row
       * 4. cập nhật số lượng product
       */

      $sql = "select a.number, a.price, b.id, b.code, b.name from pet_daklak_import_row a inner join pet_daklak_product b on a.itemid = b.id where importid = $iid";
      $query = $db->query($sql);
      $list = array();
      while ($item = $query->fetch()) {
        $list [$item['id']] = array(
      ...
      foreach ($list as $id => $item) {
        switch ($item['status']) {
          case 0:
            // remove
            $sql = "delete from pet_daklak_import_row where itemid = $id and importid = $iid";
            $db->query($sql);
            break;
          case 1:
            // insert
            $sql = "insert into pet_daklak_import_row (importid, itemid, number, price, total) values($iid, $id, $item[number], $item[price], ". ($item['number'] * $item['price']) .")";
            $db->query($sql);
            break;
          case 0:
            // update
            $sql = "update pet_daklak_import_row set number = $item[number] where itemid = $id and importid = $iid";
            $db->query($sql);
            break;
        }
      ...
      $sql = "update pet_daklak_import set total = $total, sourceid = $source where id = $iid";
      $db->query($sql);
    ...
    case 'insert-import':
      ...
      $sql = 'insert into pet_daklak_import (userid, sourceid, total, time) values('. $user_info['userid'] .', '. $source .', '. $total .', '. time() .')';
      $db->query($sql);
      $iid = $db->lastInsertId();

      foreach ($data as $id => $row) {
        $sql = 'insert into pet_daklak_import_row (importid, itemid, number, price, total) values('. $iid .', '. $id .', '. $row['number'] .', '. $row['price'] .', '. ($row['number'] * $row['price']) .')';
        $db->query($sql);

        $sql = 'update pet_daklak_product set number = number + '. $row['number'] .', buy_price = '. $row['price']. ' where id = '. $id .'';
        $db->query($sql);
      }
      $result['status'] = 1;
      $result['html'] = importContent();
    ...
    case 'update-product':
      ...
      case 'get-item':
        ...
        while ($row = $query->fetch()) {
          ...
          $xtpl->assign('price', $row['buy_price']);
          $xtpl->parse('main');
        }
        $result['status'] = 1;
